...
class AuthService {
  ...
  public async handleLogin(model: AuthDto, req: Request, res: Response): Promise<void> {
    ...
    if (!user) {
      throw new HttpException(409, `User with email ${model.email} is not exits`)
    }
  ...
  public async handleRefreshToken(refreshToken: string) : Promise<TokenData> {
    ...
    // evaluate jwt
    try {
      const decoded: Decoded = (await jwtVerifyPromisified(refreshToken, process.env.REFRESH_TOKEN_SECRET!)) as Decoded
