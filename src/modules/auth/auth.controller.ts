...
export default class AuthController {
  ...
  public getCurrentLoginUser = catchAsync(
    async (req: Request, res: Response, next: NextFunction) => {
  ...
  public refreshToken = catchAsync(
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      const tokeData: TokenData = await this.authService.handleRefreshToken(
        refreshToken
      );
      ...
      res
        .status(200)
        .json({ data: tokeData.accessToken, message: "Refresh token success" });
    }
  );
  public handleLoginSuccess = catchAsync(
    async (req: Request, res: Response, next: NextFunction) => {
      const { userId, tokenLogin } = req.body;
      await this.authService.handleLoginSuccess(userId, tokenLogin, req, res);
    }
  );
}
