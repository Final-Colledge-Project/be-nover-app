...
export default class ColumnController {
  ...
  public createColumn = catchAsync(async (req: Request, res: Response) => {
    ...
    const newColumn = await this.columnService.createColumn(model, boardId);
    res
      .status(StatusCodes.CREATED)
      .json({ data: newColumn, message: "Create column successfully" });
  });
  ...
  public getColumnByBoardId = catchAsync(
    async (req: Request, res: Response) => {
      const boardId = req.params.id;
      const userId = req.user.id;
      const columns = await this.columnService.getColumnsByBoardId(
        boardId,
        userId
      );
      res
        .status(StatusCodes.OK)
        .json({ data: columns, message: "Get columns successfully" });
    }
  );
  public updateColumn = catchAsync(async (req: Request, res: Response) => {
    ...
    res
      .status(StatusCodes.OK)
      .json({ data: updatedColumn, message: "Update column successfully" });
  });
