...
export default class LabelController {
  ...
  public createLabel = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    const model: CreateLabelDto = req.body;
    const userId = req.user.id;
    const label = await this.labelService.createLabel(model, userId);
    res.status(201).json({data: label, message: 'Create label successfully'});
  });
  public getLabelsByBoardId = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    const boardId = req.params.id;
    const labels = await this.labelService.getLabelsByBoardId(boardId);
    res.status(200).json({data: labels, message: 'Get labels successfully'});
  });
  public getLabelById = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  ...
  public updateLabel = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    const userId = req.user.id;
    const labelId = req.params.id;
  ...
  public deleteLabel = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    const userId = req.user.id;
    const labelId = req.params.id;
