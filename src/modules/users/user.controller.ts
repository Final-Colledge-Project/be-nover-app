...
export default class UserController {
  ...
  public registerUser = catchAsync(
    async (req: Request, res: Response, next: NextFunction) => {
      const model: RegisterDto = req.body;
      const user = await this.userService.createUser(model, req);
      res
        .status(201)
        .json({ data: user, message: "Register user successfully" });
    }
  );
  ...
  public uploadAvatar = catchAsync(
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      res
        .status(200)
        .json({ data: user, message: "Upload avatar successfully" });
    }
  );
  public getBulkUsers = catchAsync(
    async (req: Request, res: Response, next: NextFunction) => {
      const users = await this.userService.getBulkUsers();
      res
        .status(200)
        .json({ data: users, message: "Get bulk users successfully" });
    }
  );
  public updateProviderToken = catchAsync(
    async (req: Request, res: Response, next: NextFunction) => {
      const model: UpdateTokenDto = req.body;
      await this.userService.updateProviderToken(req.user.id, model);
