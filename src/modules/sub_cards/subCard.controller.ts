...
export default class SubCardController {
  ...
  public assignMemberToSubCard = catchAsync(async (req: Request, res: Response) => {
    const userId = req.user.id;
    const subCardId = req.params.id;
    const assigneeId = req.params.assigneeId;
    await this.subCardService.assignMemberToSubCard(userId, subCardId, assigneeId);
    res.status(StatusCodes.OK).json({ message: "Assign member to sub card successfully" });
  })
  public getAllSubCardInCard = catchAsync(async (req: Request, res: Response) => {
    const userId = req.user.id;
    const cardId = req.params.id;
    const subCards = await this.subCardService.getAllSubCardInCard(cardId, userId);
    res.status(StatusCodes.OK).json({ data: subCards, message: "Get all sub card in card successfully" });
  })
}