...
const authMiddleware = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!token) {
    return next(new HttpException(401, "You are not logged in! Please log in to get access"))
  }

  //2. Verification token
  const decoded : Decoded = await jwtVerifyPromisified(token, process.env.JWT_TOKEN_SECRET!) as Decoded
